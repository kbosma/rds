...
@Component({
  ...
  template: `
    <div class="header">
      ...
      <h1>{{ (isNew() ? 'authorities.newAuthority' : 'authorities.editAuthority') | translate }}</h1>
    </div>
    <mat-card>
      <mat-card-content>
        <form [formGroup]="form" (ngSubmit)="save()">
          <mat-form-field appearance="outline" class="full-width">
            <mat-label>{{ 'authorities.description' | translate }}</mat-label>
...
export class AuthorityDetailComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.isNew.set(!this.authorityId);
    if (this.authorityId) {
      this.authorityService.getById(this.authorityId).pipe(takeUntilDestroyed(this.destroyRef)).subscribe({
        next: (authority) => this.form.patchValue(authority),
      });
    }
  }
  ...
  save() {
    ...
    const op = this.authorityId
      ? this.authorityService.update(this.authorityId, value)
      : this.authorityService.create(value);
