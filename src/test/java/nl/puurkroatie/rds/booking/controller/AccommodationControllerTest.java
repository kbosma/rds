...
class AccommodationControllerTest extends AbstractBookingControllerTest {
    ...
    // ADMIN: GET /api/accommodations — alle accommodations over alle organisaties (>= 6)
    @Test
    void admin_findAll_returnsAllAccommodations() throws Exception {
    ...
    // ADMIN: GET /api/accommodations/{id} — elke accommodation opvraagbaar
    @Test
    void admin_findById_returnsAnyAccommodation() throws Exception {
        String token = adminToken();
        ...
        mockMvc.perform(get("/api/accommodations/" + ACCOMMODATION_PK_1)
                ...
                .andExpect(jsonPath("$.name").value("Dubrovnik Zeezicht Suite"));
    }
    ...
    // EMPLOYEE: POST /api/accommodations — 201 (heeft BOOKING_WRITE)
    @Test
    void employee_createAccommodation_returns201() throws Exception {
        String token = employeeToken();
        ...
        mockMvc.perform(post("/api/accommodations")
                        ...
                        .content(json))
                .andExpect(status().isCreated())
                .andExpect(jsonPath("$.name").value("Employee Accommodatie"))
                .andExpect(jsonPath("$.tenantOrganization").value(ORG_PUURKROATIE_ID.toString()));
    }

    // EMPLOYEE: PUT /api/accommodations/{id} eigen org — 200
    @Test
    void employee_updateAccommodation_ownOrganization_returns200() throws Exception {
        String token = employeeToken();
        ...
        mockMvc.perform(put("/api/accommodations/" + ACCOMMODATION_PK_1)
                        ...
                        .content(json))
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.name").value("Dubrovnik Zeezicht Suite Updated"));
    }
